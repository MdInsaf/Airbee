...
def require_idempotency_key(scope: str = "default"):
    ...
    def decorator(view_func: Callable) -> Callable:
        ...
        def wrapper(self, request, *args, **kwargs) -> Response:
            ...
            # Atomic: check + claim + execute + cache in single transaction
            with transaction.atomic():
                with connection.cursor() as cur:
                    # Try to INSERT. If key already exists and has response, return it.
                    cur.execute(
                        """
                    ...
                    if not is_new:
                        # Key already exists. Check if we have a cached response.
                        cur.execute(
                            """
                            SELECT response_status, response_body FROM api_idempotency_keys
                        ...
                        if row and row[0]:  # response_status exists
                            logger.info(
                                "idempotency_replay",
                                extra={
                                    "tenant_id": tenant_id,
                        ...
                        # Key exists but no response yet; conflict (concurrent request in flight)
                        return Response(
                            {
